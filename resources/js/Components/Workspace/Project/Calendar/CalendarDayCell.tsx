...
import { eachDayOfInterval, endOfMonth, isAfter, isBefore, startOfMonth } from "date-fns";
...
export function CalendarDayCell({children, date}: PropsWithChildren<Props>) {
    ...
    return (
        <div className="h-36 outline outline-bordercolor outline-1 p-2 bg-content">
            <h2 className="text-right">{date.getDate()}</h2>
            <div className="flex flex-col overflow-clip gap-2">
                {taskList.map((task) => {
                    return (
